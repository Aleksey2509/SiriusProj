...
double* segmentTreeCtor(double* values, int size);
void buildSubTree(double* segmentTree, double* values, int current, int left, int right);
void update(double* segmentTree, int index, double value, int maxSize);
int find(double* segmentTree, double randomNum, int maxSize);
void segmentTreeDtor(double* segmentTree);

double* segmentTreeCtor(double* values, int size)
{
    double* segmentTree = (double *)calloc(4 * size + 4, sizeof(double));
...
inline void update(double* segmentTree, int index, double value, int maxSize)
{
    int i = index + maxSize;
    segmentTree[i] = value;
...
inline int find(double* segmentTree, double randomNum, int maxSize)
{
    int i = 1;
    while (i >= 1)
    {
